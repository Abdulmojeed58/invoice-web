  ...
  return (
    <div>
      {label && (
        <label
          htmlFor={id}
          className="font-Montserrat text-greyPrimary text-[16px] font-medium mb-[12px] block"
        >
          {label}
        </label>
      )}
      <input
        type={type}
        id={id}
        className={`${inputClassName} rounded-[10px] w-full h-[60px] block px-5 text-[20px] bg-grey-light ${
          isError ? "border border-red-400 outline-red-400" : ""
        } ${focus ? "border border-primary outline-primary" : ""}`}
